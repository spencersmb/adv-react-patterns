...
class Toggle extends React.Component {
	...
	static Button = (props) => <Switch on={props.status} onClick={props.toggle}/>
	static On = (props) => props.status ? <div>{props.children}</div> : null
	// destructured
	static Off = ({status, children}) => !status ? <div>{children}</div> : null
...
function Usage ({
	...
	return (
		<Toggle onToggle={onToggle}>
			{/*choose order of which things are rendered*/}
